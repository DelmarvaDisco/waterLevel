...
#  - Lots of Jackson Lane and Baltimore Corner Sites filled up with data. Need to do gap filling. 
#  - XB-CH HOBO wasn't launched properly. Missing data. 
# 
#
...
#Gets rid of a few unnecessary sites from df
df <- df %>% filter(!is.na(baro_logger))
...
df<-df %>% 
  ...
  mutate(waterHeight   = pressureGauge / 9.81) %>% 
...
#Extra filtering
temp <- temp %>% 
  filter(Timestamp < "2021-06-18 16:00:00" | Timestamp > "2021-06-19 3:00:00")
...
#More filtering
temp <- temp %>%
  filter(Timestamp <= "2021-04-14 9:00:00" | Timestamp >= "2021-04-14 18:00:00")
...
temp <- temp %>%
  filter(Timestamp <= "2021-05-20 12:00:00" | Timestamp >= "2021-05-20 22:00:00")
...
temp <- temp %>% 
  filter(Timestamp <= "2021-10-18 11:00:00" | Timestamp >= "2021-10-18 20:00:00")
...
#More filtering
temp <- temp %>% 
  filter(Timestamp <= "2021-08-03 6:00:00")
