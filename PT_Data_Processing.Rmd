...
fun<-function(n){
  ...
  deploymetn<-as.POSIXct(master$Date[master$Sonde_ID==serial_number])
  ...
  temp<-temp[,c(2,3,4)]
  colnames(temp)<-c("Timestamp", "abs_pres_kPa", "temp_c")
  
  #format date_time
  temp$Timestamp<-strptime(temp$Timestamp, "%m/%d/%y %I:%M:%S %p")-time_offset
  temp$Timestamp<-format.Date(temp$Timestamp, "%m/%d/%Y %H:%M:%S")
